...
{
    public static class ModalidadBR
    {
        ...
        // Método general para validar toda la clase Modalidad
        public static void ValidarModalidad(Modalidad modalidad)
        {
            if (modalidad == null)
            {
                throw new ArgumentNullException(nameof(modalidad), "El objeto Modalidad no puede ser nulo.");
            }

            // Validar el nombre
            ValidarNombre(modalidad.TC_Nombre);

            // Validar la descripción
            ValidarDescripcion(modalidad.TC_Descripcion);
        }
    }
}
