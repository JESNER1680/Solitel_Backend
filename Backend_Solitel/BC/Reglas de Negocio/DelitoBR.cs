...
namespace BC.Reglas_de_Negocio
{
    public static class DelitoBR
    {
        ...
        // Método general para validar toda la clase Delito
        public static void ValidarDelito(Delito delito)
        {
            if (delito == null)
            {
                throw new ArgumentNullException(nameof(delito), "El objeto Delito no puede ser nulo.");
            }

            // Validar el nombre
            ValidarNombre(delito.TC_Nombre);

            // Validar la descripción
            ValidarDescripcion(delito.TC_Descripcion);
        }
    }
}
