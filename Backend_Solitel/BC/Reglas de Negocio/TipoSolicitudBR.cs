...
{
    public static class TipoSolicitudBR
    {
        ...
        // Método general para validar toda la clase TipoSolicitud
        public static void ValidarTipoSolicitud(TipoSolicitud tipoSolicitud)
        {
            if (tipoSolicitud == null)
            {
                throw new ArgumentNullException(nameof(tipoSolicitud), "El objeto TipoSolicitud no puede ser nulo.");
            }

            // Validar el nombre
            ValidarNombre(tipoSolicitud.TC_Nombre);

            // Validar la descripción
            ValidarDescripcion(tipoSolicitud.TC_Descripcion);
        }
    }
}
