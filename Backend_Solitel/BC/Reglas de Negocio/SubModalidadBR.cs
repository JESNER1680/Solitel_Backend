...
{
    public static class SubModalidadBR
    {
        ...
        // Método general para validar toda la clase SubModalidad
        public static void ValidarSubModalidad(SubModalidad subModalidad)
        {
            if (subModalidad == null)
            {
                throw new ArgumentNullException(nameof(subModalidad), "El objeto SubModalidad no puede ser nulo.");
            }

            // Validar el nombre
            ValidarNombre(subModalidad.TC_Nombre);

            // Validar la descripción
            ValidarDescripcion(subModalidad.TC_Descripcion);
        }
    }
}
