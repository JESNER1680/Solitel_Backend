...
{
    public static class CondicionBR
    {
        ...
        // Método general para validar toda la clase Condicion
        public static void ValidarCondicion(Condicion condicion)
        {
            if (condicion == null)
            {
                throw new ArgumentNullException(nameof(condicion), "El objeto Condicion no puede ser nulo.");
            }

            // Validar el nombre
            ValidarNombre(condicion.TC_Nombre);

            // Validar la descripción
            ValidarDescripcion(condicion.TC_Descripcion);
        }
    }
}
